...
mongoose
  .connect("mongodb+srv://zheedz:RasheeD4401@cluster0.euhcyzi.mongodb.net/museum_reservation", {
...
const fileFilter = (req, file, cb) => {
  ...
  if (extname && mimetype) {
    ...
    return cb('Error: Only JPG, PNG, GIF, BMP, WebP, and TIFF files are allowed!');
  }
};
...
app.get("/loggedInadmin", async (req, res) => {
  ...
  try {
    ...
    res.render("loggedInadmin", {users,search,admin,reservations,blocked,artifacts});
  } catch (error) {
...
app.post("/loggedInaccountInformation/remove-reservation/:id",async (req, res) => {
  const { id } = req.params;
  try {
    // Find the reservation by ID and remove it
    await Reservation.findByIdAndRemove(id);
...
app.put("/loggedIn/admin/reservations/:id/visitTime", async (req, res) => {
  ...
  try {
    ...
    await reservation.save();
    return res.send(`<script>alert("Time succesfully updated!"); window.location.href = "/loggedInadmin";</script>`);
  } catch (error) {
...
app.put("/loggedIn/admin/reservations/:id/numberOfVisitors",async (req, res) => {
    try {
      const reservationId = req.params.id;
...
app.put('/loggedIn/admin/artifacts/:artifactId', upload.single('updateImage'), async (req, res) => {
  ...
  try {
    ...
    if (req.file) {
      ...
      if (artifact.image) {
        const oldImagePath = path.join(__dirname, 'public', 'uploads', artifact.image);
